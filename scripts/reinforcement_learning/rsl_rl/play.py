...
def _configure_happo_play(env, checkpoint_path: str) -> bool:
    """Restore task-local HAPPO state and switch it to deterministic eval mode."""
    happo_wrapper = _find_happo_wrapper(env)
    if happo_wrapper is None:
        return False

    # RslRlVecEnvWrapper reset already built the task-local runner. Configure it
    # before any play step so warmup cannot execute the frozen outer PPO policy.
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    happo_state = checkpoint.get("platoon_happo_state") if isinstance(checkpoint, dict) else None
    if not happo_state:
        happo_wrapper.set_inference_mode(
            deterministic=True,
            disable_updates=True,
            force_happo_actions=False,
            disable_attack=True,
        )
        print(
            "[WARN] Checkpoint has no task-local HAPPO state. "
            "Internal HAPPO will not override actions; using the loaded outer RSL-RL policy."
        )
        return False
    ...
    print("[INFO] Loaded task-local HAPPO state and enabled deterministic HAPPO play mode.")
    return True
